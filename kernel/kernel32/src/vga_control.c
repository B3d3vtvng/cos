...
void vga_cursor_hide32(void) {
    outb(0x3D4, 0x0A);
    outb(0x3D5, 0x20);
}

void vga_cursor_show32(void) {
    outb(0x3D4, 0x0A);
...
void set_cursor_pos32(int row, int col){
...
void vga_print32(const char* str) {
    if (vga_text_buf_pos >= VGA_MAX) {
        ...
        set_cursor_pos32((vga_text_buf_pos - VGA_START) / 160, ((vga_text_buf_pos - VGA_START) % 160) / 2);
    }
