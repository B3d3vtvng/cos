...
static spinlock_t lock = SPINLOCK_INIT; //Lock for memory access
static void*(*liballoc_pgalloc)(size_t) = pmmalloc;
...
static struct boundary_tag* allocate_new_tag( unsigned int size )
{
	...
	struct boundary_tag *tag;
		...
		tag = (struct boundary_tag*)liballoc_pgalloc( pages );
...
void kfree(void *ptr)
{
	...
	flags_t flags = spinlock_aquire(&lock);
		...
		{
			spinlock_release(&lock, flags);		// release the lock
			return;
		}
...
void*   krealloc(void *p, size_t size)
{
	...
	flags_t flags = spinlock_aquire(&lock);		// lockit
...
void liballoc_set_pgalloc(void*(*pgalloc_func)(size_t)){
	liballoc_pgalloc = pgalloc_func;
}

void liballoc_set_pgfree(void(*pgfree_func)(void*)){
	liballoc_pgfree = pgfree_func;
}
